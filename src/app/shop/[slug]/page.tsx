...
export default async function ProductPage({
  ...
  if (related.length < 3) {
    const extras = products.filter((p) => p.slug !== product.slug && p.category !== product.category).slice(0, 3 - related.length);
    related.push(...extras);
  }
      ...
      <section className="bg-cream">
        <div className="mx-auto max-w-6xl px-6 py-12">
          <div className="grid gap-12 md:grid-cols-2">
            {/* Image */}
            <div className="relative aspect-square overflow-hidden bg-white" style={{ borderRadius: '5px' }}>
            ...
            <div>
              ...
              <h1 className="mt-2 font-serif text-4xl font-bold text-navy">{product.name}</h1>
              <p className="mt-2 text-sm text-text-light">{product.origin}</p>
              ...
              <p className="mt-6 leading-relaxed text-foreground">{product.description}</p>

              <AddToCart product={product} />

              {/* Christmas availability note */}
              {product.featuredFor?.includes("christmas") && (
                <div className="mt-4 flex items-center gap-2 border border-[#1a3a2a]/20 bg-[#e8f5ed] px-4 py-3" style={{ borderRadius: '5px' }}>
              ...
              {/* Fair pricing notice */}
              <div className="mt-6 border border-ocean/20 bg-ocean-light p-4" style={{ borderRadius: '5px' }}>
              ...
              <div className="mt-8 grid grid-cols-2 gap-4">
                <div className="border border-border bg-white p-4" style={{ borderRadius: '5px' }}>
                  <p className="mb-1 text-xs tracking-widest text-text-light uppercase">Origin</p>
                  <p className="text-sm text-navy">{product.origin}</p>
                </div>
                <div className="border border-border bg-white p-4" style={{ borderRadius: '5px' }}>
                  <p className="mb-1 text-xs tracking-widest text-text-light uppercase">Storage</p>
              ...
              <div className="mt-4 border border-border bg-white p-4" style={{ borderRadius: '5px' }}>
                <p className="mb-1 text-xs tracking-widest text-text-light uppercase">Sustainability</p>
